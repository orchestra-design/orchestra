...
import { notIsNil, uuid } from '../../helpers'

const FullScreenSection = styled('div')`
  ${tw(['flex', 'flex-col', 'h-auto', 'justify-end', 'relative', 'w-screen'])};
...
const JumboMobile = styled('div')`
  ${tw(['flex', 'items-end', 'screen:hidden', 'relative'])};
  height: 75vw;
`

const Fixed = styled('div')`
  ${tw(['screen:fixed', 'hidden', 'screen:block', 'pin-b', 'pin-l', 'pin-r'])};
`
...
const transitionGroup = data =>
  data.jumbo.map(({ jumbotitle }) => style => (
...
const TransitionHeading = ({ data, jumboCount }) => (
  <Transition
    native
    from={{ opacity: 0.0, transform: 'translateY(100%)' }}
    enter={{ opacity: 1, transform: 'translateY(0%)' }}
    leave={{ opacity: 0.0, transform: 'translateY(-50%)' }}
  >
    {transitionGroup(data)[jumboCount]}
  </Transition>
)

export const JumboSlider = connect(({ jumboCount }) => ({ jumboCount }))(
  ({ data, jumboCount }) => {
    const { jumbo } = data
    return (
      <Fragment>
